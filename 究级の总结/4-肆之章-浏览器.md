...
> 它们的划分源于浏览器和 Node.js 对异步执行顺序的优化需求

**宏任务（Macrotask）**

- **定义**：由宿主环境（如浏览器、Node.js）发起的异步任务。
- **常见类型**：
...
**为什么要细分**
- **宏任务的延迟性**：宏任务（如`setTimeout`）适合需要延迟执行或周期性执行的操作。
- **微任务的高优先级**：微任务（如`Promise`）适合需要尽快执行的异步回调（如数据处理）。
- 通过分层处理异步任务，能够优化渲染性能、保证操作的原子性，以及分离不同优先级的任务
### Web Worker
...
**区别**
* Expires设置绝对时间；Cache-Control设置相对时间，优先级更高
* Cache-Control如果设置no-store则完全跳过所有缓存；no-cache可能触发协商缓存

#### 协商缓存/对比缓存
> 向服务器发送请求，服务器会根据这个请求的request header的一些参数来判断是否命中协商缓存，如果命中，则返回304状态码并带上新的response header通知浏览器从缓存中读取资源，否则返回200和最新的数据

##### header参数
* **Last-Modified：资源的最后修改时间；计算开销较低**
...
* **Etag：资源的唯一标识（一个字符串，类似人的指纹）；计算开销较高**
...
* 为什么有了Last-Modified还需要E-tag：`解决1秒内多次修改无法检测的问题`
- Last-Modified 只能精确到秒级；适用于旧系统兼容，静态资源且不频繁修改的情况
- 
...
##### 性质
- HTML5 专门为存储来设计的两种**离线存储方法**，最大可存 5M。
...
- 不会随着 http 请求被发送到服务端。
##### 区别
- localStorage 数据会永久存储，除非代码删除或手动删除（关闭tab，刷新和关闭浏览器都不影响）
- sessionStorage 数据只存在于当前会话，当前tab栏和浏览器关闭则清空。
